...
async function fetchDailyCallCounts(extension) {
    ...
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const inboundQuery = `
                ...
                WHERE dstchannel LIKE '%${extension}%' 
            ...
            const missedQuery = `
                ...
                AND cdr.dstchannel LIKE '%${extension}%';
            `;
...
router.get('/missedCallCount', (req, res) => {
    ...
    if (!extension) {
        res.status(400).json({ error: 'Extension number is required' });
        return;
    }

    const query = `
        SELECT '${extension}' AS dst, COUNT(DISTINCT cdr.uniqueid) AS missed_call_count
        FROM asterisk_cdr_rports.cdr cdr
        WHERE cdr.uniqueid NOT IN (
            SELECT sub_cdr.uniqueid
            FROM asterisk_cdr_rports.cdr sub_cdr
...
router.get('/service-level', (req, res) => {
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE (q.event_type = 'COMPLETEAGENT' OR q.event_type = 'COMPLETECALLER') AND u.extension='${extension}'
        ) AS subquery
...
router.get('/dailyCallCounts', async (req, res) => {
    ...
    try {
        const dailyCallCounts = await fetchDailyCallCounts(extension);
